...
% TODO: informazione fine a se stessa, spostare?
...
\begin{proof}
	...
	\begin{figure}[ht]
		...
		\begin{subfigure}{0.45\textwidth}
			\centering
			\input{img/dyck_lemma_caso1.tikz}
			\subcaption{Caso 1.}
		\end{subfigure}
		\hfill
		\begin{subfigure}{0.45\textwidth}
			\centering
			\input{img/dyck_lemma_caso2.tikz}
			\subcaption{Caso $2$.}
		\end{subfigure}
...
\begin{itemize}
	\item un vettore $E$ di $k$ elementi, contenente per ogni blocco il valore della funzione di eccesso nella sua prima posizione;
	      % TODO: capire se per pioniere aperte o tutte
...
% TODO: implementazione dell'interfaccia
...
% TODO: costante rispetto a cosa?
\begin{itemize}
	\item $h$ è calcolabile in tempo costante;
	\item $h$ occupa spazio costante;
	\item fissati $U$ e $m$ è possibile estrarre uniformemente a caso una funzione di hash $h:U\to m$ (\flang{full randomness assumption}).
\end{itemize}
...
La tecnica HWHC permette il calcolo della prescritta di una data chiave senza la necessità di memorizzare tale chiave. Per fare ciò, vengono estratte casualmente due funzioni di hash $h_1,h_2:U\to m$.
...
% TODO: rendere l'esempio più chiaro
Per esempio
\begin{equation*}
	\begin{cases}
...
Adattando la procedura all'utlizzo di un $3$-ipergrafo, il rapporto $\gamma$ tra $m$ e $n$ è minimo: $m>1.23n$.

Il costo in spazio di questa struttura è quello per salvare i valori di $m$ variabili di $r$ bit. Considerato il rapporto tra $m$ e $n$ regolato da $\gamma$, tale costo è di $\gamma rn$ bit. Poiché si sta memorizzando una funzione $f:n\to 2^r$, le cui sono in numero $(2^r)^n$, l'information theoretical lower bound è $rn$ e pertanto, essendo $\gamma$ costante, la struttura è compatta.


\subsection{La struttura MWHC compressa}
Il sistema della tecnica MWHC è indeterminato, con un numero di gradi di libertà di $m-n$ che si possono semplicemente annullare.
È sufficiente quindi memorizzare $n$ valori per le variabili, sfruttando un array di $\gamma n$ bit, provvisto di struttura di rango, che indichi quali delle variabili totali sono non nulle.

Lo spazio occupato per questa struttura compressa è di $nr+\gamma n+o(n)=O(nr)$ bit, con un numero di bit per chiave di $r+\gamma$, contro i $r\gamma$ della versione non compressa.
